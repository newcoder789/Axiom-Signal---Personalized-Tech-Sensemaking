...
import { Brain, Lightbulb, History, AlertTriangle } from 'lucide-react';
...
export function MemoryEnhancedJournal({
    ...
    useEffect(() => {
        if (debouncedValue.length < 10) return;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
            ...
            <div className="md:col-span-2 flex flex-col h-full">
                ...
                <div className="mt-2 text-xs text-gray-400 flex justify-between">
                    ...
                    <span>{isSearching ? "Searching memory..." : "Memory active"}</span>
                </div>
            </div>
                        ...
                        {matches.map((match) => (
                            <div key={match.id} className="p-3 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow cursor-pointer">
                                <div className="flex justify-between items-start mb-1">
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${match.verdict === 'pursue' ? 'bg-green-100 text-green-700' :
